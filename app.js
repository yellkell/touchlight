...
var tiles = NOTES.map(function(note, i) {
  ...
  // Keyboard activation (pointer/touch is handled at the grid level so
  // a single drag can sweep across many tiles).
...
/* ============================================================
   DRAG-TO-PLAY  — press and slide across octagons to sound each
   ============================================================ */
var isPointerDown = false;
var lastDragIdx   = -1;   // last tile sounded in this drag (avoids re-triggering)
...
function dragTo(x, y, isDrag) {
  var idx = tileIdxFromPoint(x, y);
  if (idx === -1 || idx === lastDragIdx) return;
  lastDragIdx = idx;
  onTap(idx, isDrag);   // isDrag → shorter, lighter glissando note
}
...
gridEl.addEventListener('pointerdown', function(e) {
  ...
  isPointerDown = true;
  lastDragIdx = -1;
  // Release implicit pointer capture so touchmove reports the tile actually
  // under the finger rather than the one we started on.
  if (e.target.releasePointerCapture) {
    try { e.target.releasePointerCapture(e.pointerId); } catch (_) {}
  }
  dragTo(e.clientX, e.clientY, false);   // initial press = full tap note
});

gridEl.addEventListener('pointermove', function(e) {
  if (!isPointerDown) return;
  e.preventDefault();
  dragTo(e.clientX, e.clientY, true);    // sliding into new tiles = glissando
});

function endDrag() { isPointerDown = false; lastDragIdx = -1; }
...
/* ============================================================
   TAP HANDLER
   ============================================================ */
function onTap(idx, isDrag) {
  var t = tiles[idx];
  playNote(t.note.freq, isDrag ? 0.55 : 0.85);
  triggerFlash(t.oct);
  spawnRipple(t.tile);
...
function celebrate(finishedGuided) {
  // Song complete — return to free play and reset the mode button
  // to its pre-pressed (un-lit) state.
  mode = 'free';
  modeBtnEl.classList.remove('active');
